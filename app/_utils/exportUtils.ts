...
export function buildReactCode(state: FilterPanelState) {
  return [
    ...
    "  };",
    "  const fieldsetStyle = { display: \"grid\", gap: 10, margin: 0, padding: 14, border: `1px solid ${state.border}`, borderRadius: Math.max(12, state.radius - 8) };",
    ...
    "            {visibleCategories.slice(0, appliedCount).map((category) => <span key={category} style={{ border: `1px solid ${state.border}`, borderRadius: 999, padding: \"4px 10px\", color: state.accent }}>{category}</span>)}",
    "          </div>",
    "        )}",
    "      </fieldset>",
    "",
    ...
    "          <button type=\"submit\" disabled={state.disabled} style={{ border: 0, borderRadius: 12, padding: \"10px 14px\", background: state.accent, color: \"#020617\", fontWeight: 700 }}>Apply filters</button>",
    "          <button type=\"reset\" disabled={state.disabled} style={{ border: `1px solid ${state.border}`, borderRadius: 12, padding: \"10px 14px\", background: \"transparent\", color: state.foreground }}>Reset filters</button>",
    "        </div>",
    "      )}",
    "",
