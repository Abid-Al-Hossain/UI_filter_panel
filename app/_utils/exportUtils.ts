...
export function buildReactCode(state: FilterPanelState) {
  return [
    ...
    "  };",
    "  const fieldsetStyle = { display: \"grid\", gap: 10, margin: 0, padding: 14, border: `1px solid ${state.border}`, borderRadius: Math.max(12, state.radius - 8), transition: state.motion ? \"border-color 0.2s ease, opacity 0.2s ease\" : \"none\" };",
    ...
    "            {visibleCategories.slice(0, appliedCount).map((category) => <span key={category} style={{ border: `1px solid ${state.border}`, borderRadius: 999, padding: \"4px 10px\", color: state.accent, transition: state.motion ? \"background 0.15s ease, border-color 0.15s ease\" : \"none\" }}>{category}</span>)}",
    "          </div>",
    "        )}",
    "      </fieldset>",
    "",
    ...
    "          <button type=\"submit\" disabled={state.disabled} style={{ border: 0, borderRadius: 12, padding: \"10px 14px\", background: state.accent, color: \"#020617\", fontWeight: 700, transition: state.motion ? \"background 0.15s ease, opacity 0.15s ease\" : \"none\" }}>Apply filters</button>",
    "          <button type=\"reset\" disabled={state.disabled} style={{ border: `1px solid ${state.border}`, borderRadius: 12, padding: \"10px 14px\", background: \"transparent\", color: state.foreground, transition: state.motion ? \"border-color 0.15s ease, color 0.15s ease\" : \"none\" }}>Reset filters</button>",
    "        </div>",
    "      )}",
    "",
